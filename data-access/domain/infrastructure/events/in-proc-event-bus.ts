...
class InProcEventBusImpl implements EventBus {
  private eventSubscribers: { [eventType: string]: Array<(rawpayload: any) => Promise<void>> } = {};
  ...
  register<EventProps,T extends CustomDomainEvent<EventProps>>(event: new(...args:any) => T, func:(payload:T['payload']) => Promise<void>): void {
    ...
    this.eventSubscribers[event.name].push(async (rawpayload:T['payload']) => {
      console.log(`Received in-proc event ${event.name} with data ${JSON.stringify(rawpayload)}`);
      await func(rawpayload);
    });
  }
